...
public class BasicEventStateManager extends AbstractStateManager implements
	...
	private void writeToFile(String txId, String filename, LinkedHashMap<String,?> map)
	...
	private void deleteFile(String txId, String file)
	...
	public List<Message> getPersistedState(String objectId, Integer page,
		...
		if(page==null || pageSize==null){
			return this.getPersistedState(objectId);
		}
		if (page < 1 || pageSize < 1) {
			return new ArrayList<Message>(0);
		}
		
		List<Message> all = this.getPersistedState(objectId);
		List<Message> result = new ArrayList<Message>(pageSize);
		int begin=(page-1)*pageSize;
		for (int i = begin; i < (begin+pageSize); ++i) {
			if(all.size()<(i+1) || all.get(i).getTimestamp()>maxTime)
				break;
	...
	public Long getPersistedStateMessageCount(String objectId) {
		return 0L+this.getPersistedState(objectId).size();
