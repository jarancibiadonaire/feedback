...
function ClientAdapter(serverUrl, useeHTTPSession, echo) {
	...
	if (!jQuery.loadedLZString) {
		jQuery("head").append(
				jQuery("<script src='" + serverUrl
						+ "resources/js/lz-string-1.3.3-min.js' />"));
		jQuery("head").append(
				jQuery("<script src='" + serverUrl
						+ "resources/js/base64-string-v1.1.0.js' />"));
	}

};

jQuery.extend(ClientAdapter.prototype, {
	_compress : function(string) {
		var compressed = LZString.compressToBase64(string);
		compressed = Base64String.compressToUTF16(compressed);
		return compressed;
	},
	_decompress : function(compressed) {
		...
		var decompressed = Base64String.decompressFromUTF16(compressed);
		decompressed = LZString.decompressFromBase64(decompressed);
		return decompressed;
	},
	_decompressMessages : function(string) {
		var result;
		var jsonStart = '{"messages":[';
		var compressed = string.substring(jsonStart.length, string.length - 2);

		var data = compressed.split(" ,");
		for (var i = 0; i < data.length; i = i + 1) {
			data[i] = this._decompress(data[i]);
		}
		result = jsonStart + data.join() + "]}";
	...
	couple : function(handler, options) {
		...
		if (options.initMessage!=undefined) {
			data['initMessage'] = options.initMessage;
		}
		jQuery.ajax({
			type : 'POST',
			data : data,
	...
	_sendEvent : function(async, id, name, callback) {
		...
		jQuery.ajax({
			...
			async : async,
			url : this.serverUrl + "coupling/write/" + this.sessionId
		});
		return ret;
	},
	...
	start : function(delay) {
		...
		if (!delay) {
			if (this.sessionId) {
				this.source = new EventSource(this.serverUrl + "coupling/read/"
						+ this.sessionId + "/" + this.clientId);
				this.source.addEventListener('message', function(e) {
					if (jQuery.trim(e.data) != "") {
						var decompressed = context._decompressMessages(e.data);
